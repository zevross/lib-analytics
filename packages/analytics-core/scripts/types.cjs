...
const typesFromJsDocs = content
  ...
  .replace(
    /type AnalyticsInstanceConfig =/gm,
    "export interface AnalyticsInstanceConfig "
  )
  ...
  .replace(
    ...
    (_, typeName, argsDefStart) =>
      `export type ${typeName}Hook<T = ${typeName}Payload> =${argsDefStart}Context<T>`
  )
  // Convert contexts to generics accepting payload type
  .replace(
    /type (\w+)Context =(.*)ContextProps(\W)/gm,
    (_, typeName, typeDefStart, typeDefEnd) =>
      `type ${typeName}Context<T = ${typeName}Payload> =${typeDefStart}ContextProps<T>${typeDefEnd}`
  )
  // Convert context props types to generics accepting payload type
  .replace(
    /type (\w+)ContextProps =((.|\s)*?)payload: \w+/gm,
    (_, typeName, typeDefStart) =>
      `type ${typeName}ContextProps<T = ${typeName}Payload> =${typeDefStart}payload: T`
  )
...
`;
// NOTE @em843 This causes a TypeScript error with TS 2.3.4
// const typeExtensions = `
//   export type PageData<T extends string = string> = PageDataBase & Record<T, unknown>;
